document.addEventListener('DOMContentLoaded', function () {
  ...
  // --- Funções de validação e UI (Todas mantidas, sem alterações) ---
  ...
  function validarCampo(nomeCampo, valor) {
    // (A função validarCampo é mantida exatamente como a sua original)
  ...
  async function buscarCEP(cep) {
    // (Função de buscar CEP mantida, sem alterações)
    try {
  ...
  // --- Submissão (MODIFICADO PARA FETCH) ---
  form.addEventListener('submit', async function (e) {
    ...
    if (validarEtapa(2)) {
      ...
      const novoUsuario = {
        ...
        senha: campos.senha.value, // O PHP vai criptografar
        endereco: {
      ...
      try {
        ...
        if (data.success) {
          ...
          // Mostra erro vindo do servidor (ex: email já existe)
  ...
  // Função Toast (mantida, mas adaptada para tipo 'error')
